...
import type { CostLimitModel, CostTypeModel } from '~/types/general';
...
createPager(`cost-types-limits`, page, 15);
createFilter(`cost-types-limits`, {
    cost_type_name: '',
});

const router = useRouter();
const pager = computed(() => appStore.pagers[`cost-types-limits`]);
const filterObject = computed(() => appStore.filters['cost-types-limits']);
        ...
        <main>
            <OrganismFilters identifier="cost-types-limits" @changed="fetchCostLimits" :is-loading="isLoading">
                <FormKit
                    ...
                    label="Name"
                    name="cost_type_name"
                    placeholder="Select cost type name"
                /> 
            </OrganismFilters>
            <MoleculesList 
                :items="data" 
